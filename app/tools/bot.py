...
from app.schemas import MessageSchema, MessagesListSchema, RawMessageSchema

def format_llm_response_for_bd(response, role: str) -> MessageSchema:
...
async def generate_completion_async(self, messages: list[RawMessageSchema]) -> RawMessageSchema:
    messages_for_llm = [
    ...
    try:
        ...
        messages_for_base.append(
            format_llm_response_for_bd(response, "assistant")
        )

        # если в сообщении от llm есть tool_calls, то обращаемся к
        # инструменту
        if response.message.tool_calls:
            call = response.message.tool_calls[0]
            # обращаемся к инструменту
            result = markdown_search_tool(**call.function.arguments)
            # добавим результат инструмента к сообщениям для llm и к
            # сообщениям для БД
            tool_resp_content = str(result)
            messages_for_llm.append(
                {"role": "tool", "tool_name": call.function.name, "content": tool_resp_content}
            )
            ...
            messages_for_base.append(
                format_llm_response_for_bd(tool_response, "assistant")
            )
